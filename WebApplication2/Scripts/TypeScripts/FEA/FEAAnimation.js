...
var FEAAnimationClass = /** @class */ (function () {
    function FEAAnimationClass() {
        this._datasetDisp = null;
        this._geometryOrig = new Hashtable();
        this._btnStart = $('#btn-animation-disp-start');
        this._btnStop = $('#btn-animation-disp-stop');
        this._ampFactor = 0;
        this._ampCoeff = 0;
        this._ampStep = 0.2;
    ...
    FEAAnimationClass.prototype.StartAnimation = function () {
        var self = this;
        ...
        if (this._datasetDisp != null) {
            ...
            $.getJSON('/Simulation/GetAnimation?tmp=' + Number(new Date()), function (data, textStatus, jq) {
                if (!ErrorHandler.CheckJsonRes(data)) {
                    return;
                }
                self._datasetDisp = data.dataset;
                self._entIds = data.entsIds;
                self._isAnimated = true;
                self.AnimateMesh();
                self._btnStop.enableRbButton();
                App.Ribbon.GotoEditor();
    ...
    FEAAnimationClass.prototype.Reset = function () {
        this._datasetDisp = null;
        this._geometryOrig = new Hashtable();
        this._ampFactor = 0;
        this._ampCoeff = 0;
    ...
    FEAAnimationClass.prototype.AnimateMesh = function () {
        var self = this;
        if (!self._isAnimated) {
            return;
        }
        ...
        if (self._geometryOrig == null || self._geometryOrig.size() == 0) {
            self._geometryOrig = new Hashtable();
            self._ampCoeff = 0;
            var offsetEnt = 0;
        ...
        for (var idxEnt in this._entIds) {
            ...
            for (var idxNode = 0; idxNode < len; idxNode++) {
                ...
                destVert.set(origVert.x + (Ux * ampFactor * ampCoeff), origVert.y + (Uy * ampFactor * ampCoeff), origVert.z + (Uz * ampFactor * ampCoeff));
            }
            offsetEnt += mesh.geometry.vertices.length;
    ...
    FEAAnimationClass.prototype.StopAnimation = function () {
        var self = this;
        var layout = App.Layout;
        var scene = App.Layout.Scene;
        if (!self._isAnimated) {
