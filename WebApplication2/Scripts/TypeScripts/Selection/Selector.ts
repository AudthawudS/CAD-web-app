...
{
    (selItems: Array<SelectionItem>): void;
}
...
class Selector
{
    ...
    private _toolSelRect: ToolSelectRectangle;

    private _pointCloud: THREE.PointCloud;

    private _edges: THREE.Line;

    private _ents: IHashSet<THREE.Mesh>;

    private _entsOrigMaterial: IHashtable<THREE.Mesh, THREE.Material>;

    private _skipTags: IHashSet<number>;
    ...
    constructor()
    {
        ...
        this._edges = null;
        this._ents = new HashSet<THREE.Mesh>();
        this._entsOrigMaterial = new Hashtable<THREE.Mesh, THREE.Material>();
    }

    public SelectItems(selType: SelectionType)
    {
        var self = this;
        var selTags = new HashSet<number>();
        ...
        this._toolSelRect.ItemsSelected.on((items: Array<SelectionItem>) =>
        {
            if (items.length == 0)
            {
                return;
            ...
            var allTags = new HashSet<number>();
    ...
    public RemoveItems(items: Array<SelectionItem>)
    {
    ...
    public AddSelectedItems(items: Array<SelectionItem>)
    {
        // Get uniques items
    ...
    public GetSelectedMeshes(): Array<THREE.Mesh>
    {
    ...
    private UpdateVisuals()
    {
        ...
        for (var idx in this.SelectionItems)
        {
            ...
            {
                var selVert = <SelectionItemBufVert>item;
            ...
            {
                var selEnt = <SelectionItemEntity>item;
                if (!this._entsOrigMaterial.containsKey(mesh))
                {
                    this._entsOrigMaterial.put(mesh, mesh.material);
    ...
    private GetMeshSelectMaterial(): THREE.MeshBasicMaterial
    {
    ...
    private GetMeshFaceMaterial(): THREE.Material
    {
        if (Rendering.GetRenderingMode() == RenderingModeType.ShadingEdge)
        {
            var materialBasic = new THREE.MeshBasicMaterial(
                { color: 0xffffff, side: THREE.DoubleSide, vertexColors: THREE.FaceColors });
    ...
    private GetPointCloudMaterial(): THREE.PointCloudMaterial
    {
    ...
    public GetItemsByRect(rect: THREE.Box2,        selType: SelectionType,        callback: SelectionCallback)
    {
        var self = this;
        ...
        for (var idx in ents)
        {
            var ent: THREE.Object3D = ents[idx];
    ...
    public GetEntitiesByScreenRect(rect: THREE.Box2): Array<THREE.Object3D>
    {
        var self = this;
        var layout = App.Layout;

        var selItems = new Array<SelectionItem>();
        ...
        {
            var selItem: SelectionItem = selItems[idx];
    ...
    public GetEntityByScreenRect(rect: THREE.Box2): THREE.Object3D
    {
        ...
        for (var idx in ents)
        {
            var ent = <any>ents[idx];
            if (!ent.geometry)
            {
                continue;
            }
            var geometry = <THREE.Geometry>ent.geometry;
    ...
    public GetEntityByScreenPoint(pnt: THREE.Vector2, entType: EntityType): THREE.Object3D
    {
        var layout = App.Layout;
        var camera = App.Layout.Camera;
    ...
    private GetEntByRect(rect: THREE.Box2, ent: THREE.Object3D, outItems: Array<SelectionItem>)
    {
        var layout = App.Layout;
        var camera = App.Layout.Camera;
        var scene = App.Layout.Scene;
        ...
        var objData = <ObjectData>ent.userData;
        if (objData.IsSystem)
        {
            return;
        }

        var faceItems = new Array<SelectionItem>();
    ...
    public GetFacesByRect(rect: THREE.Box2, ent: THREE.Object3D, outItems: Array<SelectionItem>, raytrace: boolean, anyTag: boolean)
    {
        var layout = App.Layout;
        var camera = App.Layout.Camera;
        var scene = App.Layout.Scene;
    ...
    public GetEdgesByRect(rect: THREE.Box2,
        ent: THREE.Object3D,
        outItems: Array<SelectionItem>)
    {
        var layout = App.Layout;
        var camera = App.Layout.Camera;
        var scene = App.Layout.Scene;
    ...
    public GetNodesByRect(rect: THREE.Box2, ent: THREE.Object3D, outItems: Array<SelectionItem>, anyTags: boolean)
    {
        var layout = App.Layout;
