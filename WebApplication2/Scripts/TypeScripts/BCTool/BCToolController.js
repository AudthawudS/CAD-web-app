...
var BCToolController = /** @class */ (function () {
    // dependencies are injected via AngularJS $injector
    // controller's name is registered in Application.ts and specified from ng-controller attribute in index.html
    function BCToolController($scope) {
        this.$scope = $scope;
    ...
    BCToolController.prototype.GetSphericalRadius = function (model) {
        // Radius by Chord:  R = (H/2) + (W^2/8H)
        var h = model.DiameterShaft;
        var w = model.DiameterShaft;
        return (h / 2) + ((w * w) / (8 * h));
    };
    BCToolController.prototype.DoneEditing = function () {
        var self = this;
        self.IsLoading = true;
        ...
        $.post("/BCTool/GenerateImages", JSON.stringify(self.Model), function (data, textStatus) {
    ...
    BCToolController.prototype.Accept = function () {
        var self = this;
        $.post("/BCTool/BuildModel", JSON.stringify(self.Model), function (data, textStatus) {
            if (!ErrorHandler.CheckJsonRes(data)) {
                return;
            }
            self._scope.$apply(function () {
    ...
    BCToolController.$inject = [
        '$scope'
    ];
    return BCToolController;
}());
